...
class Path2STTransformer(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        num_genes: int = 200,
        ...
        if self.use_cell_type_proportion:
            ...
            logger.info(f"   - num_queries: 1 (multi-query fusion)")
        ...
        logger.info("Created hierarchical position embeddings for Path2ST-style input:")
        for i, dim in enumerate(self.scale_dims):
            if dim == self.num_genes:
                max_length = 1 + sum(self.scale_dims[:i]) + self.num_genes
                logger.info(f"   Scale {i} (dim={dim}): max {max_length} positions")
            else:
        ...
        logger.info("Path2STTransformer initialised successfully")
    ...
    @classmethod
    def load_checkpoint(cls, ckpt_path: str, device: str = 'cuda') -> 'Path2STTransformer':
        ...
        model = cls(
            ...
            contrast_weight=checkpoint.get('contrast_weight', 0.2),
...
Path2STModel = Path2STTransformer
